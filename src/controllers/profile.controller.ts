...
class ProfileController {
  ...
  /* Edit Avatar Profile [PATCH] */
  async handleEditAvatarProfile(req: Request, res: Response, next: NextFunction) {
    const { _id } = res.locals;
    const avatar = req.file;
    try {
      if (avatar) {
        const avatarHandled = await uploadAvatarImage(avatar);
        const result = await ProfileModel.findOneAndUpdate({ auth_id: _id }, { avatar: avatarHandled });
        next(createResponse({ profile: result }, 'Update profile successfully'));
      } else {
        const resultFind = await ProfileModel.findOne({ auth_id: _id });
        const result = await ProfileModel.findOneAndUpdate({ auth_id: _id }, { avatar: resultFind?.avatar });
        next(createResponse({ profile: result }, 'Update profile successfully'));
      }
    } catch (error) {
      next(error);
    ...
    if (resultValidate.success) {
      try {
        const result = await ProfileModel.findOneAndUpdate(
          { auth_id: _id },
          { dob: new Date(dob), firstName, lastName, subName, gender, bio },
        );
        next(createResponse({ profile: result }, 'Update profile successfully'));
      } catch (error) {
        next(error);
